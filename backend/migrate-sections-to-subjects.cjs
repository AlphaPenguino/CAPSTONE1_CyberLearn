...
async function migrateSectionsToSubjects() {
  ...
  try {
    ...
    if (sections.length > 0) {
      ...
      console.log(`✅ Copied ${sections.length} sections to subjects collection`);
    ...
    for (const quest of cyberQuests) {
      ...
      // If quest doesn't have a section/subject, assign to first subject
        ...
        console.log(`   📌 Assigning quest "${quest.title}" to subject "${sections[0].name}"`);
      }
      
      if (Object.keys(updateFields).length > 0) {
        await db.collection('cyberquests').updateOne(
          { _id: quest._id },
          updateFields.$unset ? 
            { $set: { subject: updateFields.subject }, $unset: updateFields.$unset } :
            { $set: updateFields }
        );
        updatedQuests++;
      }
    ...
    const users = await db.collection('users').find({ sections: { $exists: true, $ne: [] } }).toArray();
    ...
    for (const user of users) {
      if (user.sections && user.sections.length > 0) {
        ...
        console.log(`   📌 User ${user.username} has ${user.sections.length} subject(s)`);
        updatedUsers++;
      }
    }
    ...
    const updatedQuestsCount = await db.collection('cyberquests').countDocuments({ subject: { $exists: true } });
    
    console.log(`   Subjects collection: ${newSubjectsCount} documents`);
    console.log(`   Cyber quests with subject: ${updatedQuestsCount} documents`);
    ...
    const sampleQuests = await db.collection('cyberquests').find({ subject: { $exists: true } }).limit(3).toArray();
    ...
    console.log('   3. If everything works, you can drop the old sections collection');
