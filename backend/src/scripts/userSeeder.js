  ...
  try {
    ...
    for (const userData of usersData) {
      ...
      const newUser = new User({
        username: userData.username.toLowerCase(),
        fullName: userData.fullName || userData.username, // Fallback to username if fullName not provided
        email: userData.email.toLowerCase(),
    ...
    console.log(
      `Done! Created ${created} new users. Skipped ${skipped} existing users.`
    );
    process.exit(0);
  } catch (error) {
