  ...
  async (req, res) => {
    try {
      ...
      try {
        ...
        const imageUrl = `http://localhost:3000${uploadResponse.secure_url}`;
        ...
        await logActivity({
          ...
          action: AUDIT_ACTIONS.MODULE_CREATE,
        ...
        console.error("Local storage upload error details:", uploadError);
        ...
        if (uploadError && typeof uploadError.message === "string") {
          if (uploadError.message.includes("Failed to upload")) {
...
router.get(
  "/",
  protectRoute,
  authorizeRole(["instructor", "student", "admin"]),
  async (req, res) => {
    try {
      ...
      // Log module access for students
      if (req.user.privilege === "student") {
        ...
        await logActivity({
          ...
          username: req.user.username,
          userRole: req.user.privilege,
          action: AUDIT_ACTIONS.STUDENT_MODULE_ACCESS,
          resource: AUDIT_RESOURCES.MODULE,
          details: {
            accessType: "module_list",
            moduleCount: modules.length,
            category: req.query.category || "all",
            includeQuizzes: shouldPopulateQuizzes,
          },
          ...requestInfo,
        });
      }
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.privilege === "instructor") {
        if (module.createdBy.toString() !== req.user.id.toString()) {
          return res.status(403).json({
            message: "Access denied. You can only access modules you created.",
          });
        }
      }
...
// Add route to get module for editing
router.get(
  "/edit/:id",
  protectRoute,
  authorizeRole(["instructor", "admin"]),
  async (req, res) => {
    try {
      ...
      if (req.user.privilege === "instructor") {
        if (module.createdBy.toString() !== req.user.id.toString()) {
          return res.status(403).json({
            message: "Access denied. You can only edit modules you created.",
          });
        }
      }

      // Return full module data for editing
      res.json(module);
    } catch (error) {
      console.error("Error fetching module for editing:", error);
      res.status(500).json({ message: "Failed to fetch module for editing" });
    }
  }
);
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.privilege === "instructor") {
        if (module.createdBy.toString() !== req.user.id.toString()) {
          return res.status(403).json({
            message: "Access denied. You can only delete modules you created.",
          });
        }
      }
      ...
      // 2. Handle image deletion with local storage
      if (module.image) {
        const publicId = localStorage.extractPublicIdFromUrl(
          module.image.replace("http://localhost:3000", "")
        );
        if (publicId) {
      ...
      console.log(
        `Deleting ${quizzesInModule.length} quizzes from module ${module.title}`
      );
      ...
      res.json({
        success: true,
        message:
          "Module, associated quizzes, and progress data updated successfully",
      });
    } catch (error) {
      ...
      if (image && image !== module.image) {
        ...
        if (image.startsWith("data:image")) {
          ...
          if (module.image) {
            ...
            if (publicId) {
              try {
                await localStorage.uploader.destroy(publicId);
                console.log(
                  `Old image deleted from local storage: ${publicId}`
                );
              } catch (localStorageError) {
                console.error("Error deleting old image:", localStorageError);
          ...
          try {
            ...
            module.image = `http://localhost:3000${uploadResponse.secure_url}`;
          } catch (localStorageError) {
            console.error("Error uploading new image:", localStorageError);
            return res.status(500).json({
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `Starting system repair for ${userId ? "user " + userId : "all users"}`
      );
      ...
      if (allModules.length === 0) {
        return res
          .status(404)
          .json({ message: "No modules found in the system" });
      }
      ...
      for (const progress of progressRecords) {
        ...
        if (
          !progress.globalProgress.unlockedModules.some(
        ...
        if (
          !progress.globalProgress.currentModule ||
          !allModules.some(
            (m) =>
              m._id.toString() ===
              progress.globalProgress.currentModule.toString()
          )
        ) {
          // Find the most advanced unlocked module for this user
          const unlockedModuleIds = progress.globalProgress.unlockedModules.map(
            (id) => id.toString()
          );
          ...
          if (eligibleModules.length > 0) {
            ...
            progress.globalProgress.currentModule =
              eligibleModules[eligibleModules.length - 1]._id;
          } else {
        ...
        for (const module of allModules) {
          const hasModuleProgress = progress.moduleProgress.some(
          ...
          if (!hasModuleProgress) {
            ...
            needsSaving = true;
            console.log(
              `Created missing module progress for module ${module.title} for user ${progress.user}`
            );
          }
        }
...
async function reorderModulesAfterDeletion(deletedOrder) {
  try {
    ...
    console.log(
      `Reordering ${modulesToReorder.length} modules after deletion of order ${deletedOrder}`
    );
    ...
    const updatePromises = modulesToReorder.map((module, index) => {
      ...
      console.log(
        `Moving module '${module.title}' from order ${module.order} to ${newOrder}`
      );
...
async function repairProgressAfterModuleDeletion(
  deletedModuleId,
  deletedModuleOrder
) {
  try {
    ...
    const replacementModule =
      nextModule ||
      (await Module.findOne({ order: deletedModuleOrder - 1 }).select("_id"));
    ...
    console.log(
      `Using ${
        nextModule ? "next" : "previous"
      } module as replacement for deleted module`
    );
    ...
    for (const progress of progressRecords) {
      ...
      const unlockedModulesFiltered =
        progress.globalProgress.unlockedModules.filter(
          (id) => id.toString() !== deletedModuleId.toString()
        );

      if (
        unlockedModulesFiltered.length !==
        progress.globalProgress.unlockedModules.length
      ) {
      ...
      const completedModulesFiltered =
        progress.globalProgress.completedModules.filter(
          (item) => item.module.toString() !== deletedModuleId.toString()
        );

      if (
        completedModulesFiltered.length !==
        progress.globalProgress.completedModules.length
      ) {
