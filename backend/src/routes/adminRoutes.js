...
import AuditLog, { AUDIT_CATEGORIES } from "../models/AuditLog.js";
...
const LOGIN_ACTION_REGEX = /(login|register|signup)/i;
...
const buildCategoryFilter = (category) => {
  ...
  const normalizedCategory =
    category === "content_management"
      ...
      : ["analytics_access", "dashboard_access", "system_access"].includes(
            category
          )
        ? "admin_action"
  ...
  switch (normalizedCategory) {
    case "auth":
      return { resource: "user", action: { $regex: LOGIN_ACTION_REGEX } };
    case "user_management":
      return {
        $or: [
          ...
          {
            resource: "subject",
            action: { $regex: /student|assign|enroll|unenroll/i },
          },
          {
            resource: "user",
            $nor: [
              { action: { $regex: LOGIN_ACTION_REGEX } },
              { action: { $regex: ADMIN_USER_ACTION_REGEX } },
            ],
          },
        ],
      };
    case "content_creation":
      return { action: { $regex: /create/i } };
    case "content_modification":
      return { action: { $regex: /update|edit|archive|unarchive|import|export|upload/i } };
    case "content_deletion":
      return { action: { $regex: /delete|remove/i } };
    case "learning_activity":
      return {
        $and: [
          { action: { $not: FAILED_LOGIN_ACTION_REGEX } },
          {
            $or: [
              {
                resource: { $in: ["module", "quiz", "cyberquest"] },
                action: { $regex: /attempt|complete|access|view/i },
              },
              {
                resource: {
                  $in: [
    ...
    case "admin_action":
      return {
        $or: [
          { resource: { $in: ["system", "analytics", "dashboard"] } },
          { resource: "user", action: { $regex: ADMIN_USER_ACTION_REGEX } },
          { resource: { $exists: false } },
        ],
      };
    default:
      // Fallback: treat category as resource for backwards compatibility.
      return { resource: normalizedCategory };
  }
};
      ...
      res.json({
        ...
        data: {
          ...
          summary: {
            ...
            availableCategories: AUDIT_CATEGORIES,
          },
        },
      });
    } catch (error) {
...
router.get(
  ...
  async (req, res) => {
    try {
      const { userId, userSearch, category = "all", startDate, endDate } = req.query;
