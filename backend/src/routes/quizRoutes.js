...
router.get("/module/:moduleId", protectRoute, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch quizzes for this module" });
  }
});
    ...
    if (req.user.role === "student") {
      ...
      sanitizedQuiz.questions = sanitizedQuiz.questions.map((q) => {
        ...
        if (q.questionType === "multipleChoice") {
          ...
          ["codeSimulation", "codeImplementation"].includes(q.questionType)
        ) {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const lastQuiz = await Quiz.findOne({ module: moduleId })
        .sort({ order: -1 })
        .select("order");
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const lastQuiz = await Quiz.findOne({ module })
        .sort({ order: -1 })
        .select("order");
      const nextOrder = lastQuiz ? lastQuiz.order + 1 : 1;
      ...
      // Handle image upload to local storage if provided
      if (image) {
        try {
          console.log("Starting local storage upload for quiz image...");
          ...
          console.log("Local storage upload successful for quiz");
          imageUrl = `http://localhost:3000${uploadResponse.secure_url}`;
        } catch (localStorageError) {
          console.error(
            "Local storage upload error details:",
            localStorageError
          );
          ...
          if (
            localStorageError &&
            typeof localStorageError.message === "string"
          ) {
            if (localStorageError.message.includes("Failed to upload")) {
              errorMessage = "Quiz image upload failed. Try a smaller image.";
            } else if (localStorageError.message.includes("Invalid image")) {
          ...
          return res.status(500).json({
            ...
            error: localStorageError?.message || "Unknown upload error",
          });
        }
      }
      ...
      res
        .status(500)
        .json({ message: error.message || "Failed to create quiz" });
    }
  }
);
        ...
        if (image.startsWith("data:image")) {
          ...
          if (quiz.image) {
            const publicId = localStorage.extractPublicIdFromUrl(
              quiz.image.replace("http://localhost:3000", "")
            );
            if (publicId) {
              try {
                await localStorage.uploader.destroy(publicId);
                console.log(
                  `Old quiz image deleted from local storage: ${publicId}`
                );
              } catch (localStorageError) {
                console.error(
                  "Error deleting old quiz image:",
                  localStorageError
                );
          ...
          // Upload new image
          try {
            console.log("Uploading new quiz image to local storage...");
            ...
            quiz.image = `http://localhost:3000${uploadResponse.secure_url}`;
          } catch (localStorageError) {
            console.error("Error uploading new quiz image:", localStorageError);
            return res.status(500).json({
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      // 2. Delete associated image from local storage
      if (quiz.image) {
        const publicId = localStorage.extractPublicIdFromUrl(
          quiz.image.replace("http://localhost:3000", "")
        );
        ...
        if (publicId) {
          try {
            await localStorage.uploader.destroy(publicId);
            console.log(`Quiz image deleted from local storage: ${publicId}`);
          } catch (localStorageError) {
            console.error(
              "Error deleting quiz image from local storage:",
              localStorageError
            );
          }
        }
        ...
        switch (question.questionType) {
          ...
          case "fillInBlanks":
            isCorrect = question.blanks.every((blank, i) => {
              return (
                userAnswer[i] &&
                userAnswer[i].toLowerCase() === blank.answer.toLowerCase()
              );
            });
            break;
